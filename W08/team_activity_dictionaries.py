...
def print_death_age(people_dict):
    ...
    for person_key, person_data in people_dict.items():
        ...
        # Check if the person is still alive (death_year is 0)
        if death_year == 0:
            age_at_death = "Still alive"
        else:
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_data in marriages_dict.values():
        ...
        print(f"Marriage: {husband_name} ({husband_age_at_wedding} years old) and "
              f"{wife_name} ({wife_age_at_wedding} years old) in {wedding_year}")
