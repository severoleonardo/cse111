...
def main():
    ...
    # For each can in the parallel lists, unpack the values into the variables name, radius, height, and cost.
    for i in range(len(can_names)):
        ...
        # Call the compute_storage_efficiency and compute_cost_efficiency functions.
        store_eff = compute_storage_efficiency(radius, height)
        cost_eff  = compute_cost_efficiency(radius, height, cost)

        # Print the can size name, storage efficiency, and cost efficiency.
        print(f"{name} {store_eff:.2f} {cost_eff:.0f}")

        # If the storage efficiency of the current can size is greater than the maximum storage efficiency, save then the current can size name and its storage efficiency.
        if store_eff > max_store_eff:
            best_store = name
            max_store_eff = store_eff

        # If the cost efficiency of the current can size is greater than the maximum cost efficiency, then save the current can size name and its cost efficiency.
